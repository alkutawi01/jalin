...
const migrations = {
  ...
  "009_visual_execution_hardening": {
    async up(db: Kysely<unknown>) {
      await db.schema
        .alterTable("visual_requests")
        .addColumn("execution_mode", "text", (col) => col.defaultTo("magnific_api"))
        .execute();
      await db.schema
        .alterTable("visual_requests")
  ...
  "012_novela_bersiri_structure": {
    async up(db: Kysely<unknown>) {
      const rs = await sql<{ exists: boolean }>`SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reading_sections') AS exists`.execute(db);
      ...
      await db.schema.createIndex("reading_sections_work_position_idx").on("reading_sections").columns(["work_id", "position"]).execute().catch(() => {});

      const se = await sql<{ exists: boolean }>`SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'series') AS exists`.execute(db);
      if (se.rows[0]?.exists !== true) {
        await db.schema
          ...
          .addColumn("genre", "text")
          .addColumn("audience", "text")
          .addColumn("mode", "text", (col) => col.notNull().defaultTo("continuous").check(sql`mode in ('continuous','anthology')`))
          .addColumn("status", "text", (col) => col.notNull().defaultTo("ongoing").check(sql`status in ('ongoing','completed')`))
          .addColumn("created_at", "timestamptz", (col) => col.notNull().defaultTo(sql`now()`))
          .addColumn("updated_at", "timestamptz", (col) => col.notNull().defaultTo(sql`now()`))
          .execute();
      }

      const ee = await sql<{ exists: boolean }>`SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'series_entries') AS exists`.execute(db);
      if (ee.rows[0]?.exists !== true) {
        await db.schema
          .createTable("series_entries")
          .addColumn("id", "serial", (col) => col.primaryKey())
          .addColumn("series_id", "text", (col) => col.notNull().references("series.id").onDelete("cascade"))
          .addColumn("work_id", "text", (col) => col.notNull().unique().references("works.id").onDelete("cascade"))
          .addColumn("position", "integer", (col) => col.notNull().check(sql`position >= 1`))
          .addColumn("created_at", "timestamptz", (col) => col.notNull().defaultTo(sql`now()`))
          .addColumn("updated_at", "timestamptz", (col) => col.notNull().defaultTo(sql`now()`))
      ...
      await db.schema.createIndex("series_entries_series_position_idx").on("series_entries").columns(["series_id", "position"]).execute().catch(() => {});
    },
    async down(db: Kysely<unknown>) {
