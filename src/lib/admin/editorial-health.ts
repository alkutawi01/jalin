...
export async function getEditorialHealth(): Promise<EditorialHealth> {
  ...
  for (const work of works) {
    ...
    if (!hasAuthor) {
      health.authors.issues.push(`${work.id} missing public author`);
  ...
  for (const work of works) {
    ...
    if (work.published_revision_id) {
      ...
      if (!hasRevision) {
        health.revisions.issues.push(`${work.id} published revision missing`);
  ...
  for (const work of works) {
    ...
    if (workVisuals.length > 0) {
      health.visuals.issues.push(`${work.id}: ${workVisuals.length} visuals without credit`);
  ...
  if (translationWorks.length > 0) {
    health.translations.issues.push(`${translationWorks.length} works with legacy type`);
